...
void extractFPFHFeatures(PointCloudT::Ptr cloud, pcl::PointCloud<pcl::FPFHSignature33>::Ptr fpfh_features) {
    ...
    ne.setRadiusSearch(0.05); // Radio de búsqueda para las normales
    ne.compute(*normals);

    // Estimar descriptores FPFH
    pcl::FPFHEstimation<pcl::PointXYZ, pcl::Normal, pcl::FPFHSignature33> fpfh;
    fpfh.setInputCloud(cloud);
    fpfh.setInputNormals(normals);
    fpfh.setSearchMethod(tree);
    fpfh.setRadiusSearch(0.1); // Radio de búsqueda para FPFH
...
void pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg) {
    ...
    // Aplicar ICP con carasterícas FPFH
    ...
    // Filtrar correspondencias con RANSAC
    ...
    // Alinear con ICP usando las correspondencias filtradas
